...
const ImageProcessor = {
  ...
  async stitch(captures, pageInfo) {
    if (!Array.isArray(captures) || captures.length === 0) {
      throw new Error('No viewport captures were received.');
    }
    ...
    if (
      canvasWidth > this.MAX_CANVAS_DIMENSION
      || scaledPageHeight > this.MAX_CANVAS_DIMENSION
      || canvasWidth * scaledPageHeight > this.MAX_CANVAS_AREA
    ) {
      throw new Error(
        `Page is too large for a single stitched canvas (${canvasWidth} x ${scaledPageHeight}px). Try Fast mode, zoom out, or capture a shorter page.`
      );
    }
  ...
  _findBreakPoint(canvas, startY, targetEndY, maxHeight) {
    ...
    const scanHeight = Math.max(1, scanEnd - scanStart);
    const sampleWidth = Math.max(1, Math.min(canvas.width, 512));

    // Read pixels from a small scratch canvas, not the stitched output canvas.
    // Chrome locks context options on first use; the stitched canvas was created
    // for drawing, so asking for willReadFrequently here would be ignored and
    // Chrome would report a noisy extension warning. A dedicated readback canvas
    // keeps section detection fast and warning-free.
    const sampleCanvas = document.createElement('canvas');
    sampleCanvas.width = sampleWidth;
    sampleCanvas.height = scanHeight;
    const sampleCtx = sampleCanvas.getContext('2d', { willReadFrequently: true });
    sampleCtx.imageSmoothingEnabled = true;
    sampleCtx.imageSmoothingQuality = 'medium';
    sampleCtx.drawImage(
      canvas,
      0, scanStart, canvas.width, scanHeight,
      0, 0, sampleWidth, scanHeight,
    );

    const sampledRows = sampleCtx.getImageData(0, 0, sampleWidth, scanHeight).data;
    ...
    for (let localY = 0; localY < scanHeight; localY += 4) {
      const rowStart = localY * rowStride;
      const rowData = sampledRows.subarray(rowStart, rowStart + rowStride);
      const variance = this._rowVariance(rowData);

      if (variance < bestVariance) {
        bestVariance = variance;
        bestY = scanStart + localY;
      }
    }
